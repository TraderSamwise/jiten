...
(() => {
  ...
  // lib/share-extension/preprocessing-src.js
  var import_readability = __toESM(require_readability());
  var ShareExtensionPreprocessor = class {
    run(args) {
      try {
        const doc = document.cloneNode(true);
        const article = new import_readability.Readability(doc).parse();
        const ogImage = document.querySelector('meta[property="og:image"]')?.getAttribute("content") || document.querySelector('meta[name="og:image"]')?.getAttribute("content") || "";
        if (article) {
        ...
        const fallbackOg = document.querySelector('meta[property="og:image"]')?.getAttribute("content") || "";
        args.completionFunction({
          title: document.title || "",
          content: document.body.innerText || "",
          url: document.URL || "",
